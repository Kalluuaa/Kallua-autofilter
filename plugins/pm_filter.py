...
from utils import get_size, is_subscribed, get_poster, search_gagala, temp, get_settings, save_group_settings, get_shortlink, get_tutorial, send_all, wish
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        await query.answer(text=script.STATS.format((time.time() - BOT_START_TIME), psutil.cpu_percent(), psutil.virtual_memory().percent, humanbytes(total), humanbytes(used), psutil.disk_usage('/').percent, humanbytes(free)), show_alert=True)
        ...
        await query.message.edit_text(
            ...
            await client.edit_message_media(
                query.message.chat.id, 
                query.message.id, 
                InputMediaPhoto("https://graph.org/file/6c3f48094ae5b7abc60f4.jpg")
            )
            reply_markup = InlineKeyboardMarkup(btn)
            await query.message.edit_text(
                text="⋘ Pʟᴇᴀsᴇ Wᴀɪᴛ ⋙"
            ...
            btn = [[
                    InlineKeyboardButton("🤖 Mᴏᴠɪᴇ Rᴇᴏ̨ᴜᴇsᴛ Bᴏᴛ 🤖", url= GRP_LNK)
                ],[
                    InlineKeyboardButton('♙ Hᴏᴍᴇ', callback_data='start'),
            ...
            await client.edit_message_media(
                query.message.chat.id, 
                query.message.id, 
                InputMediaPhoto("https://graph.org/file/b469b9fe434661b255a6a.jpg")
            )
            reply_markup = InlineKeyboardMarkup(btn)
            await query.message.edit_text(
                text="●○○"
...
async def auto_filter(client, msg, spoll=False):
    ...
    if not spoll:
        ...
        if len(message.text) < 100:
            ...
            dlt = await message.reply_sticker('CAACAgUAAxkBAAJnY2URLWEHDHvSNTIs1S803o2DstJkAAKiAAPIlGQU_BpvPMzvnqweBA', reply_markup=InlineKeyboardMarkup(btn))
            search = search.lower()
            find = search.split(" ")
            search = ""
